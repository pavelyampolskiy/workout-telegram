...
export default function StatsScreen() {
  ...
  const renderContent = () => {
    ...
    // stats tab: month first, then week
    const renderPeriod = (d, label) => {
        ...
        <div className="space-y-3">
          ...
          <div className={CARD.className} style={CARD.style}>
            ...
            <div className="space-y-4">
              ...
              {cardio > 0 && <Bar label="Cardio" value={cardio} max={maxV} mounted={barMounted} />}
            </div>
          </div>
        </div>
      );
    ...
    return (
      <div className="space-y-5">
        {renderPeriod(data.month, 'This month')}
        {renderPeriod(data.week, 'This week')}
      </div>
    );
  };
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 p-5 pb-10">
        ...
        <div
          ref={tabsRef}
          className="relative flex gap-1 mb-5 bg-white/5 p-1 rounded-2xl"
        >
          <div
          ...
          {TABS.map(t => (
            <button
              ...
              className={`relative z-10 flex-1 py-2 px-2 rounded-xl text-xs font-bebas tracking-wider whitespace-nowrap transition-colors duration-200 ${
                tab === t.key ? 'text-white/92' : 'text-white/35 active:text-white/60'
              }`}
            >
              {t.label}
            </button>
