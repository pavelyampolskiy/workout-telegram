...
const TDEEScreen = () => {
  ...
  const calculateTDEE = () => {
    ...
    // Calculate macros using standard ratios (2.2g protein per kg, 1.0g fat per kg)
    const weightNum = convertWeightToKg(weight, weightUnit);
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <div className={`text-lg font-bebas tracking-wider ${TEXT_PRIMARY} mt-1`}>
                    {results.targetCalories} kcal per day • TDEE {results.tdee} • {results.goal?.name?.replace(/[^\w\s]/gi, '').trim() || 'Cutting'}
                  </div>
                </div>
