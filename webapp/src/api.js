...
export const api = {
  ...
  getHistory: (user_id, offset = 0, limit = 10, type = null, exclude = null) =>
    req('GET', `/api/history?user_id=${user_id}&offset=${offset}&limit=${limit}${type ? `&type=${type}` : ''}${exclude ? `&exclude=${exclude}` : ''}`),
  ...
  getStats: (user_id, days, calendar_week = false, calendar_month = false, calendar_year = false) =>
    req('GET', `/api/stats?user_id=${user_id}&days=${days}&calendar_week=${calendar_week ? '1' : '0'}&calendar_month=${calendar_month ? '1' : '0'}&calendar_year=${calendar_year ? '1' : '0'}`),
  ...
  getBodyMetricsReminder: (user_id) => req('GET', `/api/body-metrics/reminder?user_id=${user_id}`),
};
