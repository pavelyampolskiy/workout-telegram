...
export function ErrorScreen({ message = 'Something went wrong', onBack, onRetry, overlay = 'bg-black/70', image }) {
  return (
    <ScreenLayout overlay={overlay} image={image}>
      <div className="flex flex-col items-center justify-center min-h-screen p-5 gap-4">
        ...
        <div className="flex gap-3">
          {onBack && (
            <button onClick={onBack} className="card-press rounded-2xl px-6 py-3 font-bebas tracking-wider" style={CARD_BTN_STYLE}>
              Back
            </button>
          )}
          {onRetry && (
