...
@app.get("/api/history")
def get_history(user_id: int, offset: int = 0, limit: int = 10, type: str = None, exclude: str = None):
    rows, has_more = db_ops.get_history(user_id, offset, limit, workout_type=type, exclude_type=exclude)
    return {
        "items": [{
            "id": r["id"], "date": r["date"], "type": r["type"],
            "started_at": r["started_at"], "total_sets": r["total_sets"],
            "total_volume": r["total_volume"], "duration_min": r["duration_min"],
            "cardio_activity": r["cardio_activity"],
...
def get_achievements(user_id: int):
    ...
    if newly_earned:
        for ach in newly_earned:
            ach_type = ach.get("type", "workouts")
            actual_date = db_ops.get_achievement_unlock_date(user_id, ach_type, ach["threshold"])
            ts = actual_date or date.today().isoformat()
