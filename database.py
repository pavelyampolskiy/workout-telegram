...
# ── Active/Unfinished Workouts ────────────────────────────────────────────────

def get_active_workout(user_id: int):
    """Find an unfinished workout (has no sets recorded yet or was started today)."""
    with db() as conn:
        # Get today's workouts that might be in progress
        row = conn.execute(
            """SELECT w.* FROM workouts w
               WHERE w.user_id = ? AND w.date = ? AND w.type != 'CARDIO'
               ORDER BY w.id DESC LIMIT 1""",
            (user_id, date.today().isoformat()),
        ).fetchone()
        if row:
            # Check if workout has any exercises with sets
            exercises = conn.execute(
                "SELECT id FROM workout_exercises WHERE workout_id = ?", (row["id"],)
            ).fetchall()
            if not exercises:
                return dict(row)
            # Check if all exercises have sets completed
            for ex in exercises:
                sets = conn.execute(
...
def mark_workout_finished(workout_id: int):
    """Mark workout as finished (no-op for now, but can add finished flag later)."""
    pass
